...
public class PaginasController {
    ...
    public PaginasController() {
        this(new PaginaDAO(), new SeccionDAO(), new MediaDAO());
    }

    public PaginasController(PaginaDAO paginaDAO, SeccionDAO seccionDAO, MediaDAO mediaDAO) {
    ...
    public void routes(Javalin app) {
        ...
        app.get("/api/public/paginas/{id}", ctx -> {
            ...
            if (p == null) {
                ctx.status(404).json(Map.of("error", "Página no encontrada"));
                return;
            }
            ctx.json(p);
        });

...
public class ComentarioDAO {
      ...
      SELECT vc.ID_COMENTARIO,
             ...
             (
               SELECT rv.CALIFICACION
                 FROM AEROLINEA.RESENA_VUELO rv
                WHERE rv.ID_VUELO = vc.ID_VUELO
                  AND rv.ID_USUARIO_AUTOR = vc.ID_USUARIO
                FETCH FIRST 1 ROWS ONLY
             ) AS RATING_AUTOR
        FROM AEROLINEA.VUELO_COMENTARIO vc
        JOIN AEROLINEA.USUARIO u ON u.ID_USUARIO = vc.ID_USUARIO
       WHERE vc.ID_VUELO = ?
       ORDER BY vc.CREADA_EN ASC
    """;
  ...
  public long crear(long idVuelo, long idUsuario, String comentario, Long idPadre) throws SQLException {
    ...
    String chkVuelo = "SELECT 1 FROM AEROLINEA.VUELO WHERE ID_VUELO=?";
    String chkPadre = "SELECT 1 FROM AEROLINEA.VUELO_COMENTARIO WHERE ID_COMENTARIO=? AND ID_VUELO=?";
    String ins = """
      INSERT INTO AEROLINEA.VUELO_COMENTARIO (ID_VUELO, ID_USUARIO, ID_PADRE, COMENTARIO, CREADA_EN)
      VALUES (?,?,?,?, SYSTIMESTAMP)
    """;

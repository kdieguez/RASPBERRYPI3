...
async def list_partnerships(from_id: str | None = Query(None), to_id: str | None = Query(None), active: bool | None = Query(True)):
    q = {}
    if from_id: q["from_id"] = from_id
    if to_id: q["to_id"] = to_id
    if active is not None: q["active"] = active
    cur = partnerships.find(q, {"_id":0})
...
async def create_partnership(payload: PartnershipIn):
    if await partnerships.find_one({"_id": payload.id}):
        raise HTTPException(409, "ID already exists")
    await partnerships.create_index([("from_id",1),("to_id",1)], unique=True)
    doc = payload.model_dump(); doc["_id"] = doc.pop("id")
...
async def update_partnership(pid: str, payload: PartnershipIn):
    ...
    doc = payload.model_dump(); doc["_id"] = doc.pop("id")
    res = await partnerships.replace_one({"_id": pid}, doc)
...
async def delete_partnership(pid: str):
    res = await partnerships.delete_one({"_id": pid})

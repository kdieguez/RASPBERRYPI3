...
export default function AdminCiudades(){
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setPaises(data || []);
      } catch (e) { setErr("No se pudieron cargar países"); }
    })();
  }, []);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setCiudades(data || []);
      } catch (e) { setErr("No se pudieron cargar ciudades del país"); }
    })();
  }, [idPais]);
  ...
  const crear = async (e) => {
    ...
    try {
      await ciudadesApi.create(Number(idPais), nombre.trim());
      const { data } = await ciudadesApi.list(idPais);
      setCiudades(data || []);
        ...
        <div className="card">
          ...
          <div className="grid-2">
            <div>
              ...
              <select className="input" value={idPais} onChange={e=>setIdPais(e.target.value)}>
          ...
          <form className="form" onSubmit={crear}>
            ...
            {ok  && <div className="ok">¡Actualizado!</div>}
            <button className="btn btn-secondary" disabled={!idPais || !nombre.trim()}>Guardar</button>
          </form>
          ...
          <ul className="adm__list">
            {ciudades.map(c => (
              <li key={c.idCiudad} className="adm__row">
                <span>{c.nombre} <small className="adm__muted">({c.pais})</small></span>
                <button className={"adm__pill " + (c.activo ? "is-on" : "is-off")} onClick={()=>toggle(c.idCiudad)}>
                  {c.activo ? "Activa" : "Inactiva"}
                </button>
              </li>
            ))}
            {!idPais && <li className="adm__muted">Selecciona un país para ver sus ciudades.</li>}
          </ul>
        </div>
      </div>
    </>

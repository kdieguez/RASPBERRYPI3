...
async def ensure_cart(user_id: str) -> Dict[str, Any]:
    ...
    doc = {"idUsuario": user_id, "items": [], "total": 0.0, "fechaCreacion": now}
...
async def find_compra_detail(user_id: str, compra_id: str) -> Optional[Dict[str, Any]]:
    d = get_db()
    doc = await d[COLL_ORDERS].find_one({"_id": _oid(compra_id), "idUsuario": user_id})
    return _id_str(doc)
...
async def update_estado_compra(compra_id: str, nuevo_estado: int) -> None:
    d = get_db()
    await d[COLL_ORDERS].update_one({"_id": _oid(compra_id)}, {"$set": {"idEstado": nuevo_estado}})
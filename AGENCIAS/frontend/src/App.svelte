<script>
  ...
  import ProveedoresAdmin from "@/components/admin/proveedores.svelte";
  ...
  function headerCurrent(p) {
    ...
    if (match("/reservas", p).ok || match("/reservas/:id", p).ok)
      return "reservas";
    ...
    if (p === "/informacion/aerolineas-afiliadas") return "info-aerolineas";
  ...
  $: isAerolineasAfiliadas = match("/informacion/aerolineas-afiliadas", current).ok;

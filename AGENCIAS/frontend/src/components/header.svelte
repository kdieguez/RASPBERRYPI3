  ...
  <div class="wrap">
    ...
    <nav class="nav" aria-label="Navegación principal" style="display:flex;gap:8px;align-items:center;">
      {#if ui.header?.show_search}
        <form class="search" on:submit|preventDefault={() => (location.hash = `/vuelos?q=${encodeURIComponent(q)}`)}>
      ...
      {#if ui.header?.show_cart}
        <a class="btn ghost" href="#/carrito">Carrito</a>
      {/if}

      <a class="btn {current === 'home' ? 'primary' : 'ghost'}"   aria-current={current === 'home' ? 'page' : undefined} href="#/">Inicio</a>
      <a class="btn {current === 'vuelos' ? 'primary' : 'ghost'}" aria-current={current === 'vuelos' ? 'page' : undefined} href="#/vuelos">Vuelos</a>

      {#if $isLoggedIn}
        {#if isStaff}
          <a class="btn {current === 'users' ? 'primary' : 'ghost'}"  aria-current={current === 'users' ? 'page' : undefined} href="#/admin/users">Usuarios</a>
        {/if}
        {#if isAdmin}
          <a class="btn {current === 'portal' ? 'primary' : 'ghost'}" aria-current={current === 'portal' ? 'page' : undefined} href="#/admin/portal">Portal</a>
        {/if}
        <span style="opacity:.85">Hola, {$user?.nombres || $user?.email}</span>
        <button type="button" class="btn danger" on:click={logout}>Salir</button>
      {:else}
        <a class="btn {current === 'register' ? 'primary' : 'ghost'}" aria-current={current === 'register' ? 'page' : undefined} href="#/register">Registrarme</a>
        <a class="btn {current === 'login' ? 'primary' : 'ghost'}"    aria-current={current === 'login' ? 'page' : undefined} href="#/login">Login</a>
      {/if}
    </nav>
  </div>
</header>
...
<style>
  ...
  .brand { display:flex; align-items:center; gap:8px; font-weight: 600; letter-spacing: .4px; color:inherit; text-decoration: none; }
  .brand img { height: 28px; border-radius: 6px; }

  .btn { border: 1px solid #1f2937; background: #0b1020; color: #e5e7eb; padding: 8px 12px; border-radius: 10px; cursor: pointer; text-decoration: none; display:inline-block; }
  ...
  .search { display:flex; gap:6px; align-items:center; margin-right:6px; flex: 0 1 520px; }
